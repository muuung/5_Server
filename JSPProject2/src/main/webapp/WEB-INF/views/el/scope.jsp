...
<body>
    ...
    <pre>
        Servlet/JSP에는 기본적으로 내장되어 있는 4가지 객체가 존재
        ...
        3. session : 현재 사이트에 접속한 브라우저 당 1개씩 생성
                     ...
                     (세션에 로그인 정보를 기록해둠
                     브라우저가 종료되거나 로그아웃이 되기 전까지 계속 로그인 상태가 유지됨)

        4. application : 하나의 웹 애플리케이션 당 1개만 생성되는 객체
                         서버 시작 시 생성되며 종료 시까지 유지
                         누구든지 사용 가능
                         
        내장 객체의 우선 순위 ★★★        
        setAttribute("key", value)로 내장 객체가 값을 세팅할 때
        key 값이 중복되는 경우, \${key}로 작성하는 경우 범위가 작은 내장 객체가 높은 우선 순위를 가짐
    </pre>
