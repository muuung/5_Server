...
<body>
    ...
    <pre>
        JSP에서 자주 사용디거나 공통적으로 사용되는 Java 코드를
    ...
    <c:set var="temp" value ="임시 값" scope="request"/>
    ...
    <pre>
    	...
    	주의사항
    	1) test 속성 값 작성은 무조건 EL 구문이어야만 함
    	2) test 속성 값 "" 안에는 공백이 존재해서는 안딤
    </pre>
    ...
    <%-- session 범위에 세팅된 test이 값이 "session"인 경우 --%>
    <%--
    ...
    <pre>
    	c:choose : when, otherwise를 감싸는 태그
    			   (현재 태그 내부에 if ~ else if ~ else를 작성하겠음)
	...
	<pre>
		...
		6) varStatus : 현재 반복 상태와 관련된 정보를 제공하는 변수 선언
		   ...
		   count : 현재 몇 바퀴째인지 반복 횟수 반환 (1부터 시작)
